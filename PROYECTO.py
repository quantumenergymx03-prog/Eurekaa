    ...
    def exportar_pdf(self, e=None):
        ...
        try:
            ...
            axis_summaries_pdf, primary_entry_pdf = self._compute_axis_severity(
                ...
                env_hi_val,
                df=self.current_df,
                unit_map=self.signal_unit_map if isinstance(self.signal_unit_map, dict) else None,
            )
            ...
            chart_widths = []
            if img_time:
                chart_cells.append(Image(img_time, width=260, height=150))
                chart_widths.append(260)
            if img_fft:
                chart_cells.append(Image(img_fft, width=260, height=150))
                chart_widths.append(260)
            if chart_cells:
                chart_table = Table([chart_cells], colWidths=chart_widths, hAlign="CENTER")
            ...
            if img_env:
                ...
                if env_visible_peaks:
                    ...
                    self._apply_table_style(env_table)
            ...
            if aux_imgs:
                ...
                if len(aux_data) > 1:
                    ...
                    self._apply_table_style(aux_table)
            ...
            try:
                ...
                props.append(["Elementos (n)", self._tfv(getattr(self, 'br_n_field', None))])
                props.append(["d (mm)", self._tfv(getattr(self, 'br_d_mm_field', None))])
                props.append(["D (mm)", self._tfv(getattr(self, 'br_D_mm_field', None))])
                props.append(["Ángulo (°)", self._tfv(getattr(self, 'br_theta_deg_field', None))])
                # Máquina
                props.append(["RPM", self._tfv(getattr(self, 'rpm_hint_field', None))])
                props.append(["Línea (Hz)", (getattr(self, 'line_freq_dd', None).value if getattr(self, 'line_freq_dd', None) else '')])
                props.append(["Dientes engrane", self._tfv(getattr(self, 'gear_teeth_field', None))])
                ...
                if props:
                    ...
                    self._apply_table_style(tbl_props)
...
,
    ...
    def _export_pdf(self, e=None):
        ...
        try:
            ...
            self._apply_table_style(tbl_cover)
            ...
            if (frac_low_pdf2 + frac_mid_pdf2 + frac_high_pdf2) > 0:
                balance_row2 = [
                    Paragraph(f"Baja (0-30 Hz): {frac_low_pdf2 * 100:.1f}%", styles['Normal']),
                    Paragraph(f"Media (30-120 Hz): {frac_mid_pdf2 * 100:.1f}%", styles['Normal']),
                    Paragraph(f"Alta (>120 Hz): {frac_high_pdf2 * 100:.1f}%", styles['Normal']),
                ]
                balance_tbl2 = Table([balance_row2], colWidths=[170, 170, 170], hAlign="LEFT")
            ...
            self._apply_table_style(tbl_det)
    ...
    def _load_file_data(self, file_path):
        ...
        try:
            # 🔥 PASO CRÍTICO: Asegura que todo el estado anterior se borre por completo.
            self._reset_analysis_state_on_new_file()

            # El resto de tu lógica de carga, que ya es correcta:
            # (Leer CSV/Excel, estandarizar, etc.)
            ...
            if df_std is not None and not df_std.empty:
                ...
                try:
                    ...
                    pass
                try:
                    # Ajuste heurístico: si cualquier canal muestra magnitudes > 50 m/s²
                    # (probable dataset en mm/s² o con calibración duplicada),
                    # reescalamos todos los canales de aceleración dividiendo por 1000.
                    scale_needed = False
                    ...
                    for col in df.columns:
                        ...
                        abs_series = np.abs(series.to_numpy(dtype=float))
                        med = float(np.nanmedian(abs_series))
                        mx = float(np.nanmax(abs_series)) if abs_series.size else 0.0
                        metrics.append((col, med, mx))
                        if med > 50.0:
                            scale_needed = True
                    if scale_needed:
                        ...
                        details = ", ".join(
                            f"{name} (med={med:.2f}, max={mx:.2f})" for name, med, mx in metrics
                        )
                        self._log(
                            "Escala corregida automáticamente (÷1000) en columnas: "
                            + ", ".join(scaled_cols)
                            + f" | métricas previas: {details}"
                        )
                except Exception as scale_exc:
                    self._log(f"No se pudo ajustar la escala: {scale_exc}")
            else:
